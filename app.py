...
# --- STYLING CSS (LINIE CONTINUĂ ȘI FIXARE VIZUALĂ) ---
st.markdown("""
    <style>
    .main { background-color: #f8f9fa; }
    .scroll-container { overflow-x: auto; white-space: nowrap; padding-bottom: 15px; }
    
    /* Tabel fără spații între celule pentru continuitate maximă */
    ...
    /* Stil special pentru plecare și sosire în aceeași zi */
...
if choice == "📅 Harta":
    ...
    for cam in CAMERE_INFO.keys():
        ...
        for d in zile:
            ...
            # --- LOGICĂ ID CENTRAL REPARATĂ ---
            rez_activa = df_rez[(df_rez['camera'] == cam) & (df_rez['checkin_d'] <= d) & (df_rez['checkout_d'] >= d)]
            
            if not rez_activa.empty and bg != "bg-liber":
                r = rez_activa.iloc[0]
                # Fereastra vizibilă pe ecran
                v_start = max(r['checkin_d'], d_start)
                v_end = min(r['checkout_d'], d_end_view)
                
                # Cazul special: Rezervare de 1 noapte
                if (r['checkout_d'] - r['checkin_d']).days <= 1:
                    # ID-ul apare pe celula de check-in (sau check-out dacă in-ul e ascuns)
                    if d == r['checkin_d'] or (d == v_start and r['checkin_d'] < d_start):
                        label = f"{rez_care_pleaca.iloc[0]['id']}|{rez_care_vine.iloc[0]['id']}" if bg == "bg-schimb" else str(r['id'])
                else:
                    # Cazul mai multor nopți: Mijlocul porțiunii vizibile
                    durata_viz = (v_end - v_start).days
                    mijloc = v_start + timedelta(days=durata_viz // 2)
                    if d == mijloc:
                        label = f"{rez_care_pleaca.iloc[0]['id']}|{rez_care_vine.iloc[0]['id']}" if bg == "bg-schimb" else str(r['id'])
...
# --- 2. REZERVARE NOUĂ ---
    ...
    is_g = "Grup" in choice
    with st.form("f_add"):
        nume = st.text_input("Nume Client"); tel = st.text_input("Telefon (prefix 40)")
...
# --- 3. LISTĂ & STATISTICI ---
    ...
    if not df.empty:
        st.metric("Total Venituri", f"{df['pret_total'].sum()} RON")
